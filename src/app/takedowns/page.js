          ...
          <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            ...
            <div style={{ paddingBottom: i < steps.length - 1 ? 14 : 0, minHeight: i < steps.length - 1 ? 42 : "auto" }}>
              <div style={{
                ...
                color: step.done ? "#0F172A" : "#CBD5E1",
              }}>{step.label}</div>
              {step.date && (
                <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 1 }}>{step.date}</div>
              )}
              {!step.done && i === currentIdx + 1 && (
                <div style={{ fontSize: 11, color: "#CBD5E1", marginTop: 1, fontStyle: "italic" }}>Waiting...</div>
              )}
            </div>
          </div>
        );
    ...
    <div style={{
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid #F1F5F9" }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, color: td.status === "resolved" ? "#94A3B8" : "#92400E" }}>
            {td.status === "resolved" ? "Recovered:" : "At risk:"}
          </span>
          <span style={{
            fontSize: 16, fontWeight: 700, fontFamily: "var(--font-mono)",
            color: td.status === "resolved" ? "#16A34A" : "#DC2626",
            letterSpacing: "-0.02em",
          }}>${td.revenue.toLocaleString()}</span>
...
export default function TakedownsPage() {
  ...
  const ViewBtn = ({ icon, mode }) => (
    <button onClick={() => setView(mode)} style={{
      padding: "6px 10px", borderRadius: 6, fontSize: 16, border: "1px solid",
      cursor: "pointer", lineHeight: 1,
      background: view === mode ? "#EFF6FF" : "white",
      borderColor: view === mode ? "#BFDBFE" : "#E2E8F0",
    }}>{icon}</button>
  );
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#94A3B8", margin: "4px 0 0" }}>
            {active.length} in progress · {resolved.length} resolved
          </p>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #F1F5F9" }}>
                {["Domain", "Channel", "Status", "Revenue at Risk", "Submitted", "Days Active"].map(h => (
              ...
              {filtered.map((td, i) => {
                ...
                return (
                  <tr key={td.id} style={{
                    ...
                    opacity: mounted ? 1 : 0, animation: mounted ? `fadeIn 0.3s ease ${i * 0.03}s both` : "none",
                  }}
                  ...
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{
                        ...
                        color: td.status === "resolved" ? "#16A34A" : "#EF4444",
                      }}>${td.revenue.toLocaleString()}</span>
